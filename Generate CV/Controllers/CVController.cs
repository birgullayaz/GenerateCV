        ...
        ILogger<CVController> logger) : Controller
    {
        ...
        public IActionResult GenerateCV()
        {
            try
            {
                ...
                {
                    _logger.LogWarning("No email found in session, redirecting to home");
            ...
            {
                _logger.LogError(ex, "Error in GenerateCV action");
            ...
            [FromForm] IFormFile? photo = null)
        {
            try
            {
                ...
                // Tarih değerlerini UTC'ye çevir
                if (schools != null)
                {
                    foreach (var school in schools)
                    {
                        if (school.StartDate.HasValue)
                            school.StartDate = DateTime.SpecifyKind(school.StartDate.Value, DateTimeKind.Utc);
                        if (school.FinishDate.HasValue)
                            school.FinishDate = DateTime.SpecifyKind(school.FinishDate.Value, DateTimeKind.Utc);
                    }
                }
